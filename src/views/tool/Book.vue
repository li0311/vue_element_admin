...
<script>
import AddList from '../course/AddList'
import UpdateList from '../course/UpdateList'
// 接口
import { fetchList, deleteBook } from '@/api/book.js'
...
export default {
  ...
  created() {
    this._getBookList(this.pageNum, this.pageSize)
  },
  methods: {
    // 请求数据
    _getBookList(page, limit) {
      ...
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        deleteBook(row.id).then(res => {
          if (res.code === 20000) {
            ...
            const currentIndex = this.bookList.findIndex(item => item.id === row.id)
            this.bookList.splice(currentIndex, 1)
          }
        })
      }).catch(() => {
        this.$message({
