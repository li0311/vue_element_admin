        ...
        <el-table
          ...
          <el-table-column
                  ...
                  <template slot-scope="scope">
                    <div class="content">
                      <img :src="scope.row.value.cover" width="150px" alt="">
                      <div class="content-text">
                        <p>{{scope.row.value.title}}</p>
                        <p class="price-left">原始价格:<span>{{scope.row.price | priceFilter}}</span></p>
                        <p class="price-left">拼团价格:<span>{{scope.row.price | priceFilter}}</span></p>
                      </div>
                    </div>
                  </template>
          </el-table-column>
          <el-table-column
                  prop="groupList.p_num"
                  align="center"
                  label="成团人数"
                  width="100">
                  <template slot-scope="scope">{{scope.row.p_num}}</template>
          </el-table-column>
          <el-table-column
                  prop="groupList.expire"
                  align="center"
                  label="拼团限时（小时）"
                  width="150">
...
// 接口
import { fetchGroup, updateGroup, createGroup } from '@/api/marketing.js'
...
export default {
  ...
  methods: {
    // 请求数据
    _getGroupList(page, limit) {
      fetchGroup({page: page, limit: limit}).then(res => {
